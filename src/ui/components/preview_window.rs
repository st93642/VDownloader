...
impl PreviewWindow {
    pub fn new(parent: &ApplicationWindow, result: &SearchResult) -> Self {
        ...
        gtk4::glib::spawn_future_local(async move {
            debug!("Fetching video stream URL for: {} (platform: {:?})", url, platform);
    ...
    async fn get_video_url(url: &str, platform: &crate::core::downloader::Platform) -> Result<String, String> {
        debug!("Extracting video URL from: {} (platform: {:?})", url, platform);
        ...
        for (i, format) in formats.iter().enumerate() {
            info!("Attempt {} - Using format: {} for URL: {}", i + 1, format, url);
        ...
        } else if last_error.contains("dzen") && (last_error.contains("KeyError") || last_error.contains("exportResponse")) {
            Err("Dzen video preview not available. Try downloading instead.".to_string())
        } else if last_error.contains("ERROR") {
            let error_line = last_error.lines()
                .find(|line| line.contains("ERROR"))
                .unwrap_or("Unknown error");
            Err(format!("Preview failed: {}", error_line))
        } else {
            Err("Could not extract video URL. This video may not be supported for preview.".to_string())
        }
    }
}
