...
- On Linux, try using the `mold` linker:
...
1. Edit `src/core/downloader.rs` - Update `detect_platform()`
1. Edit `src/core/search.rs` - Update `platform_from_hint()`
1. Add tests in same file with `#[cfg(test)]`
...
1. Update `SearchView` struct and methods
1. Rebuild and test visually
...
1. Install CodeLLDB for debugging
...
1. Open project root
1. Configure Rust toolchain: Settings → Languages & Frameworks → Rust
...
1. **On PR**: Same checks plus coverage
1. **On release**: Build binaries for all platforms
...
1. [CODEBASE_INDEX.md](CODEBASE_INDEX.md) - File and module reference
1. [DEVELOPMENT_GUIDE.md](DEVELOPMENT_GUIDE.md) - How to modify code
...
1. Use faster linker: `mold` (Linux)
